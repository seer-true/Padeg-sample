...
typedef void(__stdcall* TDeInitialization);

void CallDeInitialization()
{
    if (hDLL) {
        // Получаем указатель на функцию
        TDeInitialization DeInitializationProc =
            (TDeInitialization)GetProcAddress(hDLL, "DeInitialization");

        if (DeInitializationProc) {
            // Вызываем функцию
            DeInitializationProc;
        } else {
            // Обработка ошибки - функция не найдена
            //			ShowMessage(L"Функция DeInitialization не найдена в DLL");
            cout << "Unable to load function(s)." << endl;
        }

	} else {
		// Обработка ошибки - DLL не найдена
		//		ShowMessage(L"Библиотека PadegUC.dll не найдена");
