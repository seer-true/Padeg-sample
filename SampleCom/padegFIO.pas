...
{$ELSE} stdcall; {$ENDIF}
  TGetSex = function(pMiddleName: PChar): Integer; {$IFDEF CDECL} cdecl;
{$ELSE} stdcall; {$ENDIF}
...
{$IFDEF RAISEERROR}
    raise Exception.Create('Функция ''GetExceptionsFileName'' в библиотеке ' + TPadegDLL.LibName + ' не найдена');
{$ELSE}
    Exit;
{$ENDIF}
  nLen := 1000;
  ...
  try
    ...
    if error = 0 then SetString(Result, res, nLen);
  finally
    StrDispose(res);
  end;
end;
...
{$ENDIF}
  ...
  if error <> 0 then CreateError(error);
...
begin
  ...
  else Result := pSetDictionary(PChar(FileName));
end;
...
{$ENDIF}
    ...
    if UpdateFileExcept then
      if not UpdateExceptions then Result := -1;
  end;
end;
