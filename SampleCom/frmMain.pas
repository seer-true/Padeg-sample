...
begin
  ...
  else if mResult.Lines.Count > 10 then
...
begin
  ...
  if dict <> '' then
    ...
    if Dialogs.MessageDlg('Текущий словарь "' + dict + '"' + sLineBreak + 'Загрузить новый словарь?', mtConfirmation,
...
begin
  ...
  mResult.Lines.Add('======================');
  for i := 1 to 6 do
    mResult.Lines.Add(PadegStr(i, True) + ':'#$0009 + DeclCurrency(StrToFloat(ReplaceStr(eNumber.Text, '.',
  ...
  case rgSource.ItemIndex of
    ...
    1: str := NumberToString(StrToFloat(ReplaceStr(eNumber.Text, '.', FormatSettings.DecimalSeparator)),
        cbSex.ItemIndex - 1, seDecimal.Value, cbDelZero.Checked, cbFract.Checked);
  end;

  for i := 1 to 6 do
    mResult.Lines.Add(PadegStr(i, True) + ': ' + DeclNumeral(str, i, cbSex.ItemIndex - 1, cbTypeNumeral.ItemIndex = 1,
      cbAnimate.Checked));
...
begin
  ...
  mResult.Lines.Add('NumberToString = ' + NumberToString(StrToFloat(ReplaceStr(eNumber.Text, '.',
    FormatSettings.DecimalSeparator)), cbSex.ItemIndex - 1, seDecimal.Value, cbDelZero.Checked, cbFract.Checked));

  mResult.Lines.Add('SumInWords = ' + SumInWords(ReplaceStr(eNumber.Text, '.', FormatSettings.DecimalSeparator),
    cbSex.ItemIndex - 1, seDecimal.Value, cbDelZero.Checked, cbFract.Checked));

  mResult.Lines.Add('DoubleToVerbal  = ' + DoubleToVerbal(StrToFloat(ReplaceStr(eNumber.Text, '.',
    FormatSettings.DecimalSeparator))));

  mResult.Lines.Add('DeclCurrency   = ' + DeclCurrency(StrToFloat(ReplaceStr(eNumber.Text, '.',
    FormatSettings.DecimalSeparator)), Copy(cbCurr.Text, 1, 3), cbPadegNum.ItemIndex + 1, seForms.Value));

  case rgSource.ItemIndex of
    0:
      str := DoubleToVerbal(StrToFloat(ReplaceStr(eNumber.Text, '.', FormatSettings.DecimalSeparator)));
    1:
      str := NumberToString(StrToFloat(ReplaceStr(eNumber.Text, '.', FormatSettings.DecimalSeparator)),
        cbSex.ItemIndex - 1, seDecimal.Value, cbDelZero.Checked, cbFract.Checked);
  end;

  mResult.Lines.Add(rgSource.Items[rgSource.ItemIndex] + ' DeclNumeral = ' + DeclNumeral(Str, cbPadegNum.ItemIndex + 1,
    cbSex.ItemIndex - 1, cbTypeNumeral.ItemIndex = 1, cbAnimate.Checked));
end;
